...
contract ClaimAirdrop is Script {
    address public CLAIMING_ADDRESS = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266; // default anvil address
    uint256 public CLAIM_AMOUNT = 25 ether;
    ...
    function splitSignature(bytes memory sig) public pure returns (uint8 v, bytes32 r, bytes32 s) {
        if(sig.length != 65) revert MerkleAirdrop__InvalidSignatureLength();
        assembly {  
            r := mload(add(sig, 32))
            s := mload(add(sig, 64))
